...
@section('content')
  ...
  <section class="relative bg-brand-950 overflow-hidden">
    <div class="absolute inset-0 z-0">
      <img src="{{ asset('assets/hero/portofolio.webp') }}" alt="Portofolio  Drs. Chaeroni & Rekan" class="w-full h-full object-cover opacity-40" loading="lazy" />
  ...
  <section class="py-24 bg-slate-50" id="case-studies">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      @php
        $caseGradients = ['bg-blue-800', 'bg-blue-700', 'bg-blue-600', 'bg-blue-900', 'bg-blue-800', 'bg-blue-700'];
      ...
      <div class="mt-12 grid md:grid-cols-2 gap-8" id="case-grid">
        @forelse ($caseStudies as $index => $case)
          <article class="case-card reveal bg-white rounded-3xl border border-slate-100 overflow-hidden card-hover" data-industry="{{ $case->industry }}">
            <div class="h-44 relative overflow-hidden">
              ...
              @else
                <div class="absolute inset-0 {{ $caseGradients[$index % count($caseGradients)] }}"></div>
        ...
        @empty
          <p class="text-center text-slate-500 col-span-full">Belum ada studi kasus yang tersedia.</p>
        @endforelse
      </div>
    </div>
  </section>
